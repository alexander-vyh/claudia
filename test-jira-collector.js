...
async function testCapabilityClassification() {
  ...
  mockSearchIssues.fn = async () => [
    makeIssue({ key: 'DWDEV-9407', summary: 'Terraform attribute mapping for SSO config', assigneeId: 'jira-alice', assigneeName: 'Alice' }),
  ];
  ...
  assert.strictEqual(result.tickets[0].key, 'DWDEV-9407');
...
async function testKtloClassification() {
  const db = makeMockDb(teamMembers);
  mockSearchIssues.fn = async () => [
    makeIssue({ key: 'DWS-16742', summary: 'Access Request: Bob needs Snowflake role', assigneeId: 'jira-bob', assigneeName: 'Bob', project: 'DWS' }),
  ];
...
async function testKtloByIssueType() {
  const db = makeMockDb(teamMembers);
  mockSearchIssues.fn = async () => [
    makeIssue({ key: 'DWS-16800', summary: 'Something unrelated', assigneeId: 'jira-carol', assigneeName: 'Carol', issuetype: 'Service Request', project: 'DWS' }),
  ];
...
async function testMixedTickets() {
  const db = makeMockDb(teamMembers);
  mockSearchIssues.fn = async () => [
    makeIssue({ key: 'DWDEV-9407', summary: 'Terraform attribute mapping', assigneeId: 'jira-alice', assigneeName: 'Alice' }),
    makeIssue({ key: 'DWS-16742', summary: 'Access Request: Snowflake', assigneeId: 'jira-bob', assigneeName: 'Bob', project: 'DWS' }),
    makeIssue({ key: 'DWS-16750', summary: 'Onboarding: new hire setup', assigneeId: 'jira-carol', assigneeName: 'Carol', project: 'DWS' }),
    makeIssue({ key: 'DWDEV-9410', summary: 'Implement SSO integration', assigneeId: 'jira-alice', assigneeName: 'Alice' }),
    makeIssue({ key: 'DWS-16760', summary: 'Password reset for user X', assigneeId: 'jira-bob', assigneeName: 'Bob', project: 'DWS' }),
  ];
  ...
  assert.ok(keys.includes('DWDEV-9407'), 'Terraform ticket should be capability');
  assert.ok(keys.includes('DWDEV-9410'), 'SSO ticket should be capability');
...
async function testUnmatchedAssignee() {
  const db = makeMockDb(teamMembers);
  mockSearchIssues.fn = async () => [
    makeIssue({ key: 'DWDEV-9500', summary: 'Deploy new service', assigneeId: 'jira-unknown', assigneeName: 'Unknown Person' }),
  ];
...
async function testUnassignedTicket() {
  const db = makeMockDb(teamMembers);
  mockSearchIssues.fn = async () => [
    makeIssue({ key: 'DWDEV-9501', summary: 'Migrate database schema', assigneeId: null, assigneeName: null }),
  ];
...
async function testKtloSummaryPatterns() {
  ...
  for (const summary of ktloSummaries) {
    mockSearchIssues.fn = async () => [
      makeIssue({ key: 'DWS-99', summary, assigneeId: 'jira-alice', assigneeName: 'Alice', project: 'DWS' }),
    ];
...
async function testCapabilitySummaryPatterns() {
  ...
  for (const summary of capSummaries) {
    mockSearchIssues.fn = async () => [
      makeIssue({ key: 'DWS-100', summary, assigneeId: 'jira-alice', assigneeName: 'Alice', project: 'DWS' }),
    ];
...
async function testDwdevDefaultsCapability() {
  const db = makeMockDb(teamMembers);
  mockSearchIssues.fn = async () => [
    makeIssue({ key: 'DWDEV-9999', summary: 'Update documentation for API', assigneeId: 'jira-bob', assigneeName: 'Bob' }),
  ];
...
async function testSupportIssueType() {
  const db = makeMockDb(teamMembers);
  mockSearchIssues.fn = async () => [
    makeIssue({ key: 'DWS-16900', summary: 'Help with data pipeline', assigneeId: 'jira-carol', assigneeName: 'Carol', issuetype: 'Support', project: 'DWS' }),
  ];
...
async function testEmptyTeam() {
  const db = makeMockDb([]);
  mockSearchIssues.fn = async () => [
    makeIssue({ key: 'DWDEV-100', summary: 'Deploy service', assigneeId: 'jira-alice', assigneeName: 'Alice' }),
  ];
