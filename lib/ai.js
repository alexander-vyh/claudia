...
async function assessEmailUrgency({ from, to, cc, subject, snippet, body }) {
  ...
  try {
    ...
    if (!text) {
      log.warn({ from: from?.substring(0, 80), subject }, 'AI triage: empty response');
      return null;
    }

    // Log full decision trail: input summary, raw response, token usage
    log.info({
    ...
    if (typeof result.urgent !== 'boolean' || typeof result.reason !== 'string') {
      log.warn({ result, from: from?.substring(0, 80), subject }, 'AI triage: unexpected shape');
      return null;
    }
...
async function parseRuleRefinement({ emailMeta, currentRule, userInstruction }) {
  ...
  try {
    log.info({ instruction: userInstruction, from: emailMeta.from?.substring(0, 80) }, 'AI rule refinement: sending to model');
    ...
    const text = response.content[0]?.text;
    if (!text) {
      log.warn({ from: emailMeta.from }, 'AI rule refinement: empty response');
      return null;
    }
...
const { MEETING_SUMMARY_PROMPT } = require('./meeting-prompt');

function buildMeetingSummaryPrompt({ transcript, attendees, title, durationMin }) {
  const lines = transcript.map(seg => `[${seg.speaker}] ${seg.text}`);
  const header = [
  ...
  return {
    systemMessage: MEETING_SUMMARY_PROMPT,
    userMessage: [...header, ...lines].join('\n')
  };
}

function selectMeetingModel() {
  // Sonnet for all meetings — Haiku showed systematic precision issues across
  // all 4 meeting types in A/B testing (false speaker resolution, over-extraction).
  // Cost delta is ~$0.60/month — not worth the accuracy tradeoff.
...
async function summarizeMeeting({ transcript, attendees, title, durationMin }) {
  ...
  const model = selectMeetingModel();
...
module.exports = { assessEmailUrgency, parseRuleRefinement, getClient, summarizeMeeting, buildMeetingSummaryPrompt, selectMeetingModel };
