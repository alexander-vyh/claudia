  ...
  meetingMetadata: [
    {
      ...
      date: '2026-03-10',
      attendees: ['Alexander Vyhmeister', 'Bill Price', 'Geoffrey Schuette', 'Daniel Sherr']
    }
  ],
...
assert.strictEqual(identifyTeam('Bill Price'), 'cse');
assert.strictEqual(identifyTeam('Daniel Richardson'), 'cse');
assert.strictEqual(identifyTeam('Kinski Wu'), 'cse');
assert.strictEqual(identifyTeam('Keshon Bowman'), 'desktop');
assert.strictEqual(identifyTeam('Ken Dominiec'), 'desktop');
assert.strictEqual(identifyTeam('Daniel Sherr'), 'security');
assert.strictEqual(identifyTeam('Geoffrey Schuette'), 'security');
...
assert.strictEqual(identifyTeam(null, 'U0412G376E9'), 'cse', 'Should find Bill Price by Slack ID');
...
assert.strictEqual(identifyTeam(null, null, 'billp@simpli.fi'), 'cse');
assert.strictEqual(identifyTeam(null, null, 'keshon.bowman@simpli.fi'), 'desktop');
assert.strictEqual(identifyTeam(null, null, 'geoffrey@simpli.fi'), 'security');
...
{
  const signals = [
    { type: 'jira', text: 'Default apps for first deployment', team: 'security', key: 'DWDEV-9058', topics: ['ztd'] },
    ...
    { type: 'jira', text: 'Terraform import existing Okta groups', team: 'cse', key: 'DWDEV-9012', topics: ['terraform'] },
...

{
  const ztdGroup = {
    ...
    team: 'security',
    signals: [
      { type: 'jira', text: 'Default apps for first deployment', status: 'Done', key: 'DWDEV-9058' },
...

{
  ...
  // Ken Dominiec (desktop) confirmed zero-touch working — but ZTD is a security project
...
{
  // Druva fix was done by Keshon Bowman (desktop) — should be attributed to desktop
  const result = curateForWeeklySummary(FIXTURES);
...

{
  const signals = [
    ...
    { type: 'jira', text: 'JamfPro Terraform device naming', team: 'security', key: 'DWDEV-9067', topics: ['jamfpro', 'terraform'] }
  ];
